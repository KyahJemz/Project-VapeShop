    ...
    public partial class CheckOut : Form
    {
        ...
        private void btn_BuyNow_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                sqlcon.Open();
                SqlCommand cmd = new SqlCommand("insert into Transactions(Account_Id,Firstname,Lastname,TotalPrice,TotalQuantity,Timestamp) VALUES(@Buyer_Id,@Firstname,@Lastname,@TotalPrice,@TotalQuantity,@Timestamp); SELECT SCOPE_IDENTITY()", sqlcon);
                ...
                {
                    sqlcon.Open();
                    SqlCommand cmd2 = new SqlCommand("insert into Carts(Transaction_Id,Item_Id) values (@Transaction_Id, @Item_Id)", sqlcon);
                    cmd2.Parameters.AddWithValue("@Transaction_Id", orderId);
                    cmd2.Parameters.AddWithValue("@Item_Id", int.Parse(x));
